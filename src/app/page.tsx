...
export default function Home() {
  ...
  // Scroll on the section; center via an inner min-h-full wrapper so tall content
  // can still scroll to the top (items-center on the scroll container clips it).
  const sectionClassName = "transition-all duration-1000 fixed w-full h-full left-0 overflow-y-auto";
  const sectionInnerClassName = "min-h-full flex flex-col justify-center py-20 pb-40";
  ...
  return (
    <main className={`dark transition-colors duration-1000 flex min-h-screen flex-col items-center justify-between p-4 sm:p-8 ${colors.background || ''}`.trim()}>
        <div ref={(ref: any) => menuRef.current = ref} className="fixed block right-0 top-0 w-full justify-between sm:justify-center border-b border-gray-300 bg-gradient-to-b from-zinc-200 backdrop-blur-2xl dark:border-neutral-800 dark:bg-zinc-800 dark:from-inherit lg:bg-gray-200 lg:dark:bg-zinc-800 z-10 overflow-x-auto">
